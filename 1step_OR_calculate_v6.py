...
# Output
output_dir = "./ar_analysis_output_optimized"
...
# Plotting Functions
# --------------------------------------------------------
def plot_map(data, lons, lats, title, filename, vmin=None, vmax=None, sig_mask=None, cmap='coolwarm', extend='both'):
...
def plot_pooled_or_ci(results_by_lag, region_name, region_zh_name):
    """Plots a bar chart of pooled Odds Ratios with 95% CI error bars."""
    ...
    plt.title(f"Pooled OR for {region_zh_name}")
...
def run_regional_analysis(region_name, region_dict,
    ...
    for L in lags:
        ...
        plot_map(OR_ci_lower, lons_region, lats_region, f"OR 95% CI Lower Bound ({region_dict['zh_name']}, lag={L}d)",
                 f"cc_or_ci_lower_lag{L}_{region_name}", vmin=0.5, vmax=vmax, sig_mask=sig_mask, cmap='plasma')
        plot_map(OR_ci_upper, lons_region, lats_region, f"OR 95% CI Upper Bound ({region_dict['zh_name']}, lag={L}d)",
                 f"cc_or_ci_upper_lag{L}_{region_name}", vmin=0.5, vmax=vmax, sig_mask=sig_mask, cmap='plasma')

    plot_pooled_or_ci(results_by_lag, region_name, region_dict['zh_name'])
    ...
    # Plotting
    plot_map(fisher_results["p_ext_ar"], lons_region, lats_region, f"P(E|AR) ({region_dict['zh_name']})",
    ...
    plot_map(fisher_results["lift"], lons_region, lats_region, f"Lift [P(E|AR)/P(E|no AR)] ({region_dict['zh_name']})",
    ...
    plot_map(fisher_results["lift_ci_lower"], lons_region, lats_region,
             f"Lift 95% CI Lower Bound ({region_dict['zh_name']})",
    ...
    plot_map(fisher_results["lift_ci_upper"], lons_region, lats_region,
             f"Lift 95% CI Upper Bound ({region_dict['zh_name']})",
